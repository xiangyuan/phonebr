...
public class ContactManagerAnother extends ContactFactory {
	...
	public ContactManagerAnother(Context context) {
		super(context);
	...
	public IContact[] getAllContacts() {
		...
		// Cursor cursor = contentResolver.query(Data.CONTENT_URI, null, null,
		// null, Data._ID);
		// String[] cols = cursor.getColumnNames();
		// int count = cursor.getCount();
		//
		// ArrayList<HashMap<String, String>> dataList = new
		// ArrayList<HashMap<String, String>>(count);
		// HashMap<String, String> colValues;
		// if (cursor.moveToNext()) {
		// colValues = new HashMap<String, String>(cols.length);
		//
		// for(int i = 0; i < cols.length; i++) {
		// String name = cols[i];
		// String value = cursor.getString(i);
		//
		// colValues.put(name, value);
		// }
		// dataList.add(colValues);
		// }
	...
	public int update(IContact contact, IContact oldContact) {
		...
		ContentValues values = new ContentValues();
		{
			...
			if (values.size() > 0)
				uCount = contentResolver.update(uri, values, null, null); // ����Ϣ����
		}
		long raw_id = 0;
		if (contact.getRawId() != null)
		...
		if (!contactName.hasName(contact)) {
			contactName.delete(raw_id, contact); // ɾ��������
	...
	@Override
	public List<Uri> insert(IContact[] contact) {
		return null;
	}
}
